...
// ═══════════════════════════════════════════════════════════════
// CIRCUIT 3: DEGENERACY KILLS GAP — 64 QUBIT (2×4 LATTICE)
// Tests: MultipleZeroKillsGap (Lean output56d)
//
// IDENTICAL to Circuit 0 baseline EXCEPT:
//   After initialization, CNOT copies force components 0→1
//   and 2→3 DEGENERATE at ALL 8 sites.
//   This creates eigenvalue multiplicity (double zeros)
//   across the entire lattice before the Hamiltonian acts.
//
// If confinement drops → degeneracy kills spectral gap
// → experimental evidence for MultipleZeroKillsGap
// → via proved chain → FischerForward → RH ∧ AllZerosSimple
//
...
// ─── STEP 1: INITIALIZE ALL QUBITS (identical to baseline) ───
// Site (0,0): qubits 0-7
...
// ═══════════════════════════════════════════════════════════════
// STEP 1.5: FORCE DEGENERACY — ALL 8 SITES
// CNOT copy: component 0 → component 1 (forces q[s+0] = q[s+1])
// CNOT copy: component 2 → component 3 (forces q[s+2] = q[s+3])
// This creates double eigenvalues across the ENTIRE lattice.
// Components 4-7 remain free (non-degenerate control).
// ═══════════════════════════════════════════════════════════════
// Site (0,0)
cx q[0], q[1]; cx q[2], q[3];
// Site (0,1)
cx q[8], q[9]; cx q[10], q[11];
// Site (0,2)
cx q[16], q[17]; cx q[18], q[19];
// Site (0,3)
cx q[24], q[25]; cx q[26], q[27];
// Site (1,0)
cx q[32], q[33]; cx q[34], q[35];
// Site (1,1)
cx q[40], q[41]; cx q[42], q[43];
// Site (1,2)
cx q[48], q[49]; cx q[50], q[51];
// Site (1,3)
cx q[56], q[57]; cx q[58], q[59];
...
// ─── STEP 2: KINETIC TERM (identical to baseline) ───
...
// ─── STEP 3: NEAREST-NEIGHBOR INTERACTIONS (identical to baseline) ───
// Horizontal row 0
...
// ─── STEP 4: INTRA-SITE INTERACTIONS (identical to baseline) ───
// Site (0,0)
...
// ─── STEP 5: PLAQUETTE TERMS (identical to baseline) ───
...
// ─── MEASUREMENT: SUBSET ONLY (identical to baseline) ───
